...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult Index()
        {
            ShoppingCart cart = GetShoppingCartFromCookie();
            if (cart != null && cart.Items.Any())
            {
                ViewBag.CheckCart = cart;
            }
        ...
        /*private ShoppingCart GetShoppingCartFromCookie()
        {
            ...
            if (cartCookie != null)
            {
                return JsonConvert.DeserializeObject<ShoppingCart>(cartCookie.Value);
            }
            return null;
        }*/
        private ShoppingCart GetShoppingCartFromCookie()
        {
            ...
            if (cartCookie != null)
            {
                var decryptedData = Decrypt(cartCookie.Value, EncryptionKey);
                return JsonConvert.DeserializeObject<ShoppingCart>(decryptedData);
            }
            return null;
        }


        /*private void SaveShoppingCartToCookie(ShoppingCart cart)
        {
            var cartCookie = new HttpCookie("Cart", JsonConvert.SerializeObject(cart));
            cartCookie.Expires = DateTime.Now.AddDays(7);
            Response.Cookies.Add(cartCookie);
        }*/
        private void SaveShoppingCartToCookie(ShoppingCart cart)
        {
            var encryptedData = Encrypt(JsonConvert.SerializeObject(cart), EncryptionKey);
            var cartCookie = new HttpCookie("Cart", encryptedData);
            cartCookie.Expires = DateTime.Now.AddDays(7);
            Response.Cookies.Add(cartCookie);
        }
        ...
        {
            var cartCookie = new HttpCookie("Cart");
